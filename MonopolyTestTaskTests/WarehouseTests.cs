...
namespace MonopolyTestTaskTests
{
    public class WarehouseTests
    {
        ...
        public void If_LessThen3Pallets_ThenDoesNotThrow()
        {
            ...
            var actualPallet1 = warehouse.Pallets[0];
            Assert.Equal(pallet2, actualPallet1);
            var actualPallet2 = warehouse.Pallets[1];
            Assert.Equal(pallet1, actualPallet2);

            Assert.Equal(2, warehouse.Top3PalletsWithLongestExpirationDate.Count);
            actualPallet1 = warehouse.Top3PalletsWithLongestExpirationDate[0];
            Assert.Equal(pallet1, actualPallet1);
            actualPallet2 = warehouse.Top3PalletsWithLongestExpirationDate[1];
